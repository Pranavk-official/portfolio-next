...
function FooterSection() {
  ...
  const socialLinks = dockItems.filter(
    (item) => item.id !== "separator" && item.id !== "blog" && item.id !== "resume"
  );

  return (
    <footer
      className="footer-bg relative border 2xl:h-[550px] h-fit lg:pb-20 w-[95%] mx-auto rounded-lg overflow-hidden radial-gradient-bg
    ...
    >
        ...
        <div className="sm:block flex sm:mt-0 mt-4 gap-2 sm:w-auto w-full sm:space-y-2 relative z-10">
          {socialLinks.map((social) => (
            <Link
              ...
              className="bg-background sm:w-auto w-full grid place-content-center 2xl:h-40 h-32 2xl:p-10 p-5 rounded-lg hover:bg-muted transition-colors"
      ...
      <div className="lg:flex hidden">
        <AnimatedText text="PRANAV K" className="2xl:text-[11rem] text-[12vw]" />
      </div>
    </footer>
  );
}
